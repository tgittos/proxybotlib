	...
	public class StarCraftFrame:System.Windows.Forms.Panel
	{
		...
		protected override void  OnPaint(System.Windows.Forms.PaintEventArgs g_EventArg)
		{
			...
			if (drawStartSpots)
			{
				...
				foreach(StartingLocation location in proxyBot.getStartingLocations())
				{
			...
			if (drawResources)
			{
				SupportClass.GraphicsManager.manager.SetColor(g, System.Drawing.Color.FromArgb(0, 255, 255));
				//UPGRADE_ISSUE: The following fragment of code could not be parsed and was not converted. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1156'"
				foreach(Unit unit in proxyBot.getUnits())
				{
			...
			if (drawResources)
			{
				SupportClass.GraphicsManager.manager.SetColor(g, System.Drawing.Color.FromArgb(0, 128, 0));
				//UPGRADE_ISSUE: The following fragment of code could not be parsed and was not converted. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1156'"
				foreach(Unit unit in proxyBot.getUnits())
				{
					if (unit.getType().getId() == Constants.Resource_Vespene_Geyser)
					{
						g.fillRect(unit.getX() * tileSize, panelHeight + unit.getY() * tileSize, unit.getType().getTileWidth() * tileSize, unit.getType().getTileHeight() * tileSize);
					}
				}
			}
			
			// enemy units
			SupportClass.GraphicsManager.manager.SetColor(g, System.Drawing.Color.FromArgb(255, 0, 0));
			//UPGRADE_ISSUE: The following fragment of code could not be parsed and was not converted. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1156'"
			foreach(Unit unit in proxyBot.getUnits())
			{
				if (unit.getPlayerID() == proxyBot.EnemyID)
				{
					g.fillRect(unit.getX() * tileSize, panelHeight + unit.getY() * tileSize, unit.getType().getTileWidth() * tileSize, unit.getType().getTileHeight() * tileSize);
				}
			}
			...
			foreach(Unit unit in proxyBot.getUnits())
			{
				if (unit.getPlayerID() == proxyBot.PlayerID)
				{
					g.fillRect(unit.getX() * tileSize, panelHeight + unit.getY() * tileSize, unit.getType().getTileWidth() * tileSize, unit.getType().getTileHeight() * tileSize);
				}
			}
			...
			g.DrawString("" + proxyBot.Player.Minerals, SupportClass.GraphicsManager.manager.GetFont(g), SupportClass.GraphicsManager.manager.GetBrush(g), 25, 20 - SupportClass.GraphicsManager.manager.GetFont(g).GetHeight());
			...
			SupportClass.GraphicsManager.manager.SetColor(g, System.Drawing.Color.FromArgb(0, 0, 0));
			//UPGRADE_TODO: Method 'java.awt.Graphics.drawString' was converted to 'System.Drawing.Graphics.DrawString' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javaawtGraphicsdrawString_javalangString_int_int'"
			g.DrawString("" + proxyBot.Player.Gas, SupportClass.GraphicsManager.manager.GetFont(g), SupportClass.GraphicsManager.manager.GetBrush(g), 125, 20 - SupportClass.GraphicsManager.manager.GetFont(g).GetHeight());
			
			// supply
			SupportClass.GraphicsManager.manager.SetColor(g, System.Drawing.Color.FromArgb(0, 0, 0));
			//UPGRADE_TODO: Method 'java.awt.Graphics.drawString' was converted to 'System.Drawing.Graphics.DrawString' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javaawtGraphicsdrawString_javalangString_int_int'"
			g.DrawString((proxyBot.Player.SupplyUsed / 2) + "/" + (proxyBot.Player.SupplyTotal / 2), SupportClass.GraphicsManager.manager.GetFont(g), SupportClass.GraphicsManager.manager.GetBrush(g), 200, 20 - SupportClass.GraphicsManager.manager.GetFont(g).GetHeight());
			...
			if (drawIDs)
			{
				...
				SupportClass.GraphicsManager.manager.SetFont(g, new System.Drawing.Font("ariel", textSize, System.Drawing.FontStyle.Regular));
				//UPGRADE_ISSUE: The following fragment of code could not be parsed and was not converted. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1156'"
				foreach(Unit unit in proxyBot.getUnits())
				{
